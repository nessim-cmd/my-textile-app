...
export default function PlanningDetailPage() {
    ...
    const handleStatusChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        if (!planning) return
        setPlanning({ ...planning, status: e.target.value as any })
    }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const updatedResponse = await fetch(`/api/planning/${planning.id}`)
            const updatedPlanning = await updatedResponse.json()
            setPlanning(updatedPlanning)
            setInitialPlanning(updatedPlanning)
        } catch (error) {
