...
const ExportPDF: React.FC<ExportPDFProps> = ({ exporte, totalsExport }) => {
  ...
  useEffect(() => {
    const fetchClientDetails = async () => {
      if (exporte.clientName) {
        try {
          const response = await fetch('/api/client');
          const clients: Client[] = await response.json();
          const matchedClient = clients.find((c) => c.name === exporte.clientName);
  ...
  exportData.push({
    label: 'Mode Paiement',
    value:
      exporte.modePaiment === 1
        ? 'Virement bancaire'
        : exporte.modePaiment === 2
        ? 'Chèque'
        : exporte.modePaiment === 3
        ? 'Espèce'
        : 'Non défini',
  });
        ...
        <div className="p-8" ref={exportRef}>
          <div className="flex justify-between items-center text-sm">
            ...
            <div className="text-right uppercase">
              <p className="badge badge-ghost">Export ° {exporte.id}</p>
              <p className="my-2">
                <strong>Date </strong>
          ...
          <div>
            ...
            <div>
              ...
              {client && (
                <>
                  {client.address && <p className="text-sm  text-gray-500 w-56">{client.address}</p>}
                  {client.phone1 && <p className="text-sm  text-gray-500 w-56">Tél: {client.phone1}</p>}
                  {client.email && <p className="text-sm  text-gray-500 w-56">Email: {client.email}</p>}
                </>
              )}
            </div>
          </div>
          ...
          <div className="mt-4 space-y-2 text-md">
            <div className="flex justify-between">
              ...
              <div>{totalsExport.totalHT.toFixed(2)} €</div>
            </div>

            {exporte.vatActive && (
              <div className="flex justify-between">
                <div className="font-bold">TVA {exporte.vatRate} %</div>
                <div>{totalsExport.totalVAT.toFixed(2)} €</div>
              </div>
            )}

            <div className="flex justify-between">
              <div className="font-bold">Total Toutes Taxes Comprises</div>
              <div className="badge badge-accent">{totalsExport.totalTTC.toFixed(2)} €</div>
            </div>
          </div>
