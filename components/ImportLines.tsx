...
const ImportLines: React.FC<Props> = ({ declaration, setDeclaration }) => {
  useEffect(() => {
    console.log("ImportLines - Declaration changed:", JSON.stringify(declaration, null, 2));
  }, [declaration]);
  ...
  return (
    <div className="card bg-base-200 p-4">
      ...
      {declaration.models.length === 0 ? (
        <p className="text-gray-500">Aucun modèle disponible. Ajoutez un modèle pour commencer.</p>
      ) : (
        declaration.models.map(model => (
          <div key={model.id} className="mb-6">
            ...
            <p className="text-sm text-gray-500 mb-2">
              Accessories count: {model.accessories.length}
            </p>
            {model.accessories.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="table table-zebra">
                  ...
                  <tbody>
                    {model.accessories
                      .filter(acc => acc.modelId === model.id || !acc.modelId) // Fallback for empty modelId
                      .map(acc => (
                        <tr key={acc.id}>
                          <td>
                            <input
